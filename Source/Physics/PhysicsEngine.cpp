	...
	void PhysicsEngine::update(const double delta)
	{
		for (auto& currentDynamicObject : m_dynamicObjects)
		{
			if (currentDynamicObject->getCurrentVelocity() > 0)
			{
				...
				const auto newPosition = currentDynamicObject->getCurrentPosition() + static_cast<float>(currentDynamicObject->getCurrentVelocity() * delta) * currentDynamicObject->getCurrentDirection();
