...
Level::Level(const std::vector<std::string>& levelDescription)
{
	...
	m_playerRespawn_1 = { BLOCK_SIZE * (m_width * 0.5f - 1), BLOCK_SIZE * 0.5f };
	m_playerRespawn_2 = { BLOCK_SIZE * (m_width * 0.5f + 3), BLOCK_SIZE * 0.5f };
	m_enemyRespawn_1  = { BLOCK_SIZE,						 BLOCK_SIZE * m_height - BLOCK_SIZE*0.5f };
	m_enemyRespawn_1  = { BLOCK_SIZE*(m_width*0.5f + 1),     BLOCK_SIZE * m_height - BLOCK_SIZE * 0.5f };
	m_enemyRespawn_1  = { BLOCK_SIZE * m_width,              BLOCK_SIZE * m_height - BLOCK_SIZE * 0.5f };

	m_mapObjects.reserve(m_width * m_height + 4);
	unsigned int currentBottomOffset = BLOCK_SIZE * (m_height - 1) + BLOCK_SIZE * 0.5f;
	for (const std::string& currentRow : levelDescription)
	{
		...
		for (const char currentElement : currentRow)
		{
			...
			case 'O':
				...
				break;
			default:
				m_mapObjects.emplace_back(createGameObjectFromDiscription(currentElement, glm::vec2(currentLeftOffset, currentBottomOffset), glm::vec2(BLOCK_SIZE, BLOCK_SIZE), 0.0f));
				break;
			}
	...
	// bottom border
	m_mapObjects.emplace_back(std::make_shared<Border>( glm::vec2(BLOCK_SIZE, 0.0f), glm::vec2(m_width* BLOCK_SIZE, BLOCK_SIZE/2.0f), 0.f, 0.f));

	// top border
	m_mapObjects.emplace_back(std::make_shared<Border>(glm::vec2(BLOCK_SIZE, m_height * BLOCK_SIZE + 0.5f * BLOCK_SIZE), glm::vec2(m_width * BLOCK_SIZE, BLOCK_SIZE / 2.0f), 0.f, 0.f));
	// left border
	m_mapObjects.emplace_back(std::make_shared<Border>(glm::vec2(0.0f, 0.0f), glm::vec2(BLOCK_SIZE, BLOCK_SIZE*(m_height +1)), 0.f, 0.f));
	// right border
	m_mapObjects.emplace_back(std::make_shared<Border>(glm::vec2((m_width+1) * BLOCK_SIZE, 0.0f), glm::vec2(BLOCK_SIZE*2.0f, BLOCK_SIZE * (m_height + 1)), 0.f, 0.f));
...
void Level::render() const
{
	...
	{
		if (currentMapObject && currentMapObject)
		{
