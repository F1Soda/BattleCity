...
void AIComponent::update(const double delta)
{
	...
	std::vector<Tank::EOrientation> freeCeil = getFreedomTankOrientation(false);
	if (m_lastFramePositionTank == glm::ivec2(m_pParentTank->getCurrentPosition().x, m_pParentTank->getCurrentPosition().y) && true)
	{
	...
	case AIComponent::EEnemyState::Ñhaotic:
		...
		if (correctedPositionTank.x % 16 == 0 && correctedPositionTank.y % 16 == 0 && true)
		{
			updateObjectInCheckerBox();
			freeCeil = getFreedomTankOrientation(true);
